...
class Equipment extends Component {
	...
	componentDidMount() {
		...
		const { countryOption, makeOption, manufacturerOption, allBusinessUnits, fuelOption, ...baseState } = this.state;
	...
	handleAssetChange = assetDetails => {
		this.setState({ capDetails: assetDetails, capDataloading: false, equipmentName: assetDetails.itemName, errors: { equipmentName: "", equipmentPhysicalCode: "", yearOfManufacture: "" }, touched: { equipmentName: false, equipmentPhysicalCode: false, yearOfManufacture: false } });
	}
	...
	validateForm = () => {
		...
		for (var input in errors) {
			...
			if (input === "equipmentName") {
				...
				else if (this.state.yearOfManufacture !== "" && this.state.yearOfManufacture.length !== 4) {
					errors.yearOfManufacture = "Year of Manufacture must be 4 digit!";
				}
				else {
					errors.yearOfManufacture = "";
	...
	handleSubmit = evt => {
		...
		if (isValid) {
			Swal.fire({
				title: 'Are you sure?',
				text: 'You want to create this euipment details',
				icon: 'warning',
				...
				if (result.value) {
					...
					var apiEnd = "equipment/equipment_create";
						...
						.then((resp) => {
							...
							if (resp.success === true) {
								...
								Swal.fire({
									icon: 'success',
									title: 'Success',
									text: 'Equipment Details created successfully',
								})
						...
						<form onSubmit={this.handleSubmit}>
							<div className="card-body">
								...
								{this.state.tab === "warranty" &&
									<>
										<div className="form-group row">
											...
											<div className="col-lg-6">
												<input className="form-control form-control-sm" type="number" name="warrantyHour" id="warrantyHour" value={this.state.warrantyHour} onChange={this.handleInputOnChange} />
											</div>
										</div>
								...
								{this.state.tab === "manufacturer" &&
									<Manufecture onManufactureChange={this.onManufactureChange} />
								}
								{this.state.tab === "fuel" &&
									<>
										<div className="form-group row">
