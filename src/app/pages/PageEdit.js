...
class PageEdit extends Component {
    ...
    componentDidMount() {
        ...
        this.setState({ idPages: this.props.match.params.id });
        this.getAllMenu();
        this.getPageEditInfo(id);
    }
    ...
    getPageEditInfo(idPages) {
        fetch(process.env.REACT_APP_API_URL + "page_edit/" + idPages,
        getGetRequestOptions())
        .then((resp) => {
            return resp.json()
        })
        .then((resp) => {
            this.setState({
                pagesTitle: resp.data.pagesTitle,
                pagesLink: resp.data.pagesLink,
                iconPath: resp.data.iconPath,
                pagesType: resp.data.pagesType,
    ...
    handleSubmit = evt => {
        ...
        if (isValid) {
            Swal.fire({
                title: 'Are you sure?',
                text: 'You want to update this page',
                icon: 'warning',
