...
class LogBookFuelEdit extends Component {
	constructor(props) {
		super(props);
		this.state = {
	...
	componentDidMount() {
		...
		this.getAllBu();
		if (this.props.fuelLogId !== undefined) {
			this.getFuelLogDetails(this.props.fuelLogId);
			this.setState({ fuelLogId: this.props.fuelLogId });
		}
	...
	getFuelLogDetails(fuelLogId) {
		fetch(process.env.REACT_APP_API_URL + "log_book/fuel_edit/" + fuelLogId, 
		getGetRequestOptions())
		.then((resp) => {
			return resp.json()
		})
		.then((resp) => {
			this.setState({
				...
				logDate: resp.logDate,
				fuelOfficer: resp.fuel_officer,
				fuelQty: resp.fuelQty,
				fuelType: resp.equipment.fuelTypeName,
				touched: {
					logDate: true,
					fuelQty: true
				}
	...
	handleSubmit = evt => {
		...
		if (isValid) {
			Swal.fire({
				title: 'Are you sure?',
				text: "You want to update this fuel disbursment!",
				icon: 'warning',
				...
				if (result.value) {
					...
					const formData = {
                        businessUnit: process.env.REACT_APP_EMD_BU_ID,
                        fuelLogId: this.state.fuelLogId,
					...
					var apiEnd = "log_book/fuel_update/" + this.state.fuelLogId;
					...
					fetch(process.env.REACT_APP_API_URL + apiEnd, requestOptions)
						...
						.then((resp) => {
							...
							if (resp.success === true) {
								...
								this.props.onUpdateFuelLog({ id: resp.data.id, rid: Math.random() });
							}
							else {
								//var errorsMessage = "";
								var errorsMessage = [];
					...
					<div className="card-body">
						<div className="ml-12">
							...
							<div className="form-group row">
								...
								<div className="col-lg-6">
									<AsyncSelect
										...
										isDisabled={this.state.equipment !== ""}
									/>
								</div>
							</div>
							...
							<div className="form-group row">
								...
								<div className="col-lg-6">
									<input type="date" name="logDate" id="logDate" value={this.state.logDate} onChange={this.onChangeLogDate} className={`form-control form-control-md ${touched.logDate === true ? (errors.logDate === "" ? 'is-valid' : 'is-invalid') : ''}`} readOnly />
...
export default LogBookFuelEdit;