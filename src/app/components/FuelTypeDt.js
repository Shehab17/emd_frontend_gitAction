...
const url = process.env.REACT_APP_API_URL + "fuel_type_dt";

class FuelTypeDt extends Component {
    constructor(props) {
        super(props);
    ...
    componentDidUpdate(prevProps) {
        if (this.props.addnewFuelType.id !== prevProps.addnewFuelType.id) // Check if it's a new user, you can also use some unique property, like the ID  (this.props.user.id !== prevProps.user.id)
        {
            ...
            return this.state.entities.data.map((value, key) => {
                return <tr key={key} >
                    ...
                    <td style={{ "textAlign": "center" }}>
                        <Button className="btn btn-icon btn-light btn-hover-primary btn-sm mx-3" onClick={() => this.props.onAction(value.id, value.type)}>
