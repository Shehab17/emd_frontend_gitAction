...
const url = process.env.REACT_APP_API_URL + "project_location_dt";

class ProjectLocationDt extends Component {
    constructor(props) {
        super(props);
    ...
    componentDidUpdate(prevProps) {
        if (this.props.addnewProjectLocation.id !== prevProps.addnewProjectLocation.id) {
            ...
            return this.state.entities.data.map((value, key) => {
                return <tr key={key} >
                    ...
                    <td style={{ "textAlign": "center" }}>
                        <Button className="btn btn-icon btn-light btn-hover-primary btn-sm mx-3" onClick={() => this.props.onAction(value.id, value.projectId, value.name, value.details, value.status)}>
...
export default ProjectLocationDt;