...
const columns = [
    { 'value': 'id', 'title': 'Id' },
    { 'value': 'roleName', 'title': 'Role Name' },
    { 'value': 'details', 'title': 'Details' },
    { 'value': 'activeShowStatus', 'title': 'Active Status' },
    { 'value': 'action', 'title': 'Action' },
];

const url = process.env.REACT_APP_API_URL + "role_management_dt";

class RoleManagementDt extends Component {
    constructor(props) {
        super(props);
    ...
    componentDidUpdate(prevProps) {
        if (this.props.addNewRoleManagement.id !== prevProps.addNewRoleManagement.id) // Check if it's a new user, you can also use some unique property, like the ID  (this.props.user.id !== prevProps.user.id)
        {
            ...
            return this.state.entities.data.map((value, key) => {
                return <tr key={key} >
                    ...
                    <td className="text-center">
                        <Button className="btn btn-icon btn-light btn-hover-primary btn-sm mx-3" onClick={() => this.props.onAction(value.id, value.roleName, value.details, value.activeStatus)}><span className="svg-icon svg-icon-md svg-icon-primary">
                            <SVG
...
export default RoleManagementDt;