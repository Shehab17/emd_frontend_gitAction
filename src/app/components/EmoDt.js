...
const columns = [
    ...
    { 'value': 'emoStatus', 'title': 'Status' }
];

const url = process.env.REACT_APP_API_URL + "emo/emo_dt";

class EmoDt extends Component {
    constructor(props) {
        super(props);
    ...
    getApprovalPermission() {
        ...
        fetch(process.env.REACT_APP_API_URL + "emo_edit_permission", {
			method: "GET",
            ...
            return this.state.entities.data.map((value, key) => {
                return <tr key={key} >
                    <td>
                        ...
                        {(value.emoStatus !== "parking_emo" && this.state.approvalPermission != null) &&
                            <Link to={`/emo-edit/${value.idEmo}`}>
                                <Button className="btn btn-icon btn-light btn-hover-primary btn-sm mx-3">
                                    <span className="svg-icon svg-icon-md svg-icon-primary">
                                        <SVG
                    ...
                    <td className="text-center">
                        {value.emoStatus === "emo" && <span className="label label-lg label-primary label-inline mr-2">Created</span>}
                        {(value.emoStatus === "receive_emo" || value.emoStatus === "deny_emo") && <span className="label label-lg label-warning label-inline mr-2">Received</span>}
                        {value.emoStatus === "equipment_release" && <span className="label label-lg label-success label-inline mr-2">Released</span>}
                        {value.emoStatus === "parking_emo" && <span className="label label-lg label-info label-inline mr-2">Parked</span>}
                    </td>
                </tr>
            })
        } else {
