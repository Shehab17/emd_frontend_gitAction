...
const columns = [
	{ 'value': 'logEntryTrackId', 'title': 'Id' },
...
const url = process.env.REACT_APP_API_URL + "log_book/list";

class LogBookDt extends Component {
	constructor(props) {
		super(props);
	...
	dataList() {
		if (this.state.entities.data.length) {
			return this.state.entities.data.map((value, key) => {
				return <tr key={key} >
					<td className="text-center">{value.logEntryTrackId}</td>
					...
					<td className="text-center">
						<Link to={`/log-book-details/${value.logEntryTrackId}`}>
