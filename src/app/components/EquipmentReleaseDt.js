...
const columns = [
    { 'value': 'idEquipmentRelease', 'title': 'Id' },
    { 'value': 'releaseNo', 'title': 'Release No' },
    { 'value': 'fromBusinessUnit', 'title': 'From Business Unit' },
    { 'value': 'customerName', 'title': 'Customer' },
    { 'value': 'action', 'title': 'Action' }
];

const url = process.env.REACT_APP_API_URL + "equipment/equipment_release_dt";

class EquipmentReleaseDt extends Component {
    constructor(props) {
        super(props);
    ...
    getApprovalPermission() {
        ...
        fetch(process.env.REACT_APP_API_URL + "equipment_release_edit_permission", {
			method: "GET",
    ...
    dataList() {
        if (this.state.entities.data.length) {
            return this.state.entities.data.map((value, key) => {
                return <tr key={key} >
                    <td className="text-center">{value.idEquipmentRelease}</td>
                    <td className="text-center">{value.releaseNo}</td>
                    <td className="text-center">{value.fromBusinessUnit}</td>
                    <td className="text-center">{value.customerName}</td>
                    <td className="text-center">
                        { this.state.approvalPermission != null && 
                            <Link to={`/release-equipment-edit/${value.idEquipmentRelease}`}>
                        ...
                        <Link to={`/release-equipment-details/${value.idEquipmentRelease}`}>
                            <Button className="btn btn-icon btn-light btn-hover-primary btn-sm mx-3">
                                <span className="svg-icon svg-icon-md svg-icon-primary">
                                    <SVG
...
export default EquipmentReleaseDt;