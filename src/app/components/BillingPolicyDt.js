...
const columns = [
	{ 'value': 'idBillingPolicy', 'title': 'Id' },
	{ 'value': 'policyName', 'title': 'Policy Name' },
	{ 'value': 'policyDesc', 'title': 'Description' },
	{ 'value': 'status', 'title': 'Status' },
	{ 'value': 'createDate', 'title': 'Create Date' },
	{ 'value': 'action', 'title': 'Action' }
];

const url = process.env.REACT_APP_API_URL + "billing_policy_dt";

class BillingPolicyDt extends Component {
	constructor(props) {
		super(props);
	...
	dataList() {
		if (this.state.entities.data.length) {
			return this.state.entities.data.map((value, key) => {
				return <tr key={key} >
					<td className="text-center">{value.idBillingPolicy}</td>
					<td className="text-center">{value.policyName}</td>
					<td className="text-center">{value.policyDesc}</td>
					<td className="text-center">{value.status}</td>
					<td className="text-center">{value.createDate}</td>
					<td className="text-right pr-0">
						<Link to={`/billingsetup-billingpolicy-edit/${value.idBillingPolicy}`}>
						...
						<></>
						<Link to={`/billingsetup-policy-details/${value.idBillingPolicy}`}>
							<Button
...
export default BillingPolicyDt;