...
const url = process.env.REACT_APP_API_URL + "project_billing_policy_dt";

class ProjectBillingPolicyDt extends Component {
    constructor(props) {
        super(props);
    ...
    componentDidUpdate(prevProps) {
        if (this.props.addnewProjectBillingPolicy.id !== prevProps.addnewProjectBillingPolicy.id) {
        ...
        if (this.state.entities.data.length) {
            return this.state.entities.data.map((value, key) => {
                return <tr key={key} >
                    ...
                    <td className="text-center">{value.policy}</td>
                    <td className="text-center">{value.status}</td>
                    <td style={{ "textAlign": "center" }}>
                        <Button className="btn btn-icon btn-light btn-hover-primary btn-sm mx-3" onClick={() => this.props.onAction(value.id, value.policy)}>
...
export default ProjectBillingPolicyDt;