...
const columns = [
	...
	{ 'value': 'wacc', 'title': 'Wacc' },
	{ 'value': 'action', 'title': 'Action' },
];

const url = process.env.REACT_APP_API_URL + "interest_rate_dt";

class InterestRateDt extends Component {
	constructor(props) {
		super(props);
	...
	componentDidUpdate(prevProps) {
		if (this.props.addnewInterestRate.id !== prevProps.addnewInterestRate.id) // Check if it's a new user, you can also use some unique property, like the ID  (this.props.user.id !== prevProps.user.id)
		{
		...
		if (this.state.entities.data.length) {
			return this.state.entities.data.map((value, key) => {
				return <tr key={key} >
					...
					<td className="text-center">{value.rate}</td>
					<td className="text-center">{value.effectiveDate}</td>
					<td className="text-center">{value.wacc}</td>
					<td className="text-center">
						<Button className="btn btn-icon btn-light btn-hover-primary btn-sm mx-3" onClick={() => this.props.onAction(value.id, value.rate, value.effectiveDate, value.wacc)}><span className="svg-icon svg-icon-md svg-icon-primary">
							<SVG
