...
const columns = [
    { 'value': 'teamId', 'title': 'Id' },
    { 'value': 'teamName', 'title': 'Team Name' },
    { 'value': 'teamMembers', 'title': 'Team Members' },
    { 'value': 'action', 'title': 'Action' },
];

const url = process.env.REACT_APP_API_URL + "maintenance_team/team_dt";

class MaintenanceTeamDt extends Component {
    constructor(props) {
        super(props);
    ...
    componentDidUpdate(prevProps) {
        if (this.props.addnewMaintenanceTeam.teamId !== prevProps.addnewMaintenanceTeam.teamId) // Check if it's a new user, you can also use some unique property, like the ID  (this.props.user.id !== prevProps.user.id)
        {
    ...
    dataList() {
        if (this.state.entities.data.length) {
            return this.state.entities.data.map((value, key) => {
                return <tr key={key} >
                    <td className="text-center">{value.teamId}</td>
                    <td className="text-center">{value.teamName}</td>
                    <td className="text-center">{value.teamMembers}</td>
                    <td className="text-center">
                        <Button className="btn btn-icon btn-light btn-hover-primary btn-sm mx-3" onClick={() => this.props.onAction(value.teamId, value.teamName, value.teamMembers)}><span className="svg-icon svg-icon-md svg-icon-primary">
                            <SVG
...
export default MaintenanceTeamDt;