...
const columns = [
    ...
    { 'value': 'projectName', 'title': 'Business Unit' },
    { 'value': 'month', 'title': 'Month' },
    { 'value': 'year', 'title': 'Year' },
    { 'value': 'action', 'title': 'Action' }
];

const url = process.env.REACT_APP_API_URL + "emd/log_book_import_to_do_list";

class LogBookImportToDoList extends Component {
    constructor(props) {
        super(props);
        ...
        if (this.state.entities.data.length) {
            return this.state.entities.data.map((value, key) => {
                return <tr key={key} >
                    ...
                    <td className="text-center">{value.month}</td>
                    <td className="text-center">{value.year}</td>
                    <td className="text-center">
                        <Link to={{pathname:'/log-book-import',info:{ details : JSON.stringify(value)}}}>
                            <Button type="button" className="btn btn-outline-success btn-sm"><i className="flaticon-file-1"></i> Import Log Book</Button>
                        </Link>
                    </td>
                </tr>
            })
...
export default LogBookImportToDoList;