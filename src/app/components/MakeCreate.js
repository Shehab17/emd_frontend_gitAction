...
class MakeCreate extends Component {
    constructor(props) {
        super(props);
        this.state = {
            id: "",
    ...
    componentDidUpdate(prevProps) {
        if (this.props.id !== prevProps.id || this.props.name !== prevProps.name) {
            ...
            this.getMakeDetails(this.props.id);
        }

    }

    getMakeDetails(id) {
        fetch(process.env.REACT_APP_API_URL + "make_details/" + id, 
    ...
    handleInputOnChange = evt => {
        ...
        if (evt.target.value === "") {
            errors[evt.target.name] = "Make Name is required!";
        }
        else {
    ...
    validateForm = () => {
        ...
        for (var input in errors) {
            ...
            if (input === "name") {
                if (this.state.name === "") {
                    errors.name = "Make Name is required!";
                }
                else {
                    errors.name = "";
                }
    ...
    handleSubmit = evt => {
        ...
        if (isValid) {
            if (this.state.id !== "") {
                var text = 'You want to update this Make';
            }
            else {
                text = 'You want to create this Make';
            }
            Swal.fire({
                title: 'Are you sure?',
                text: text,
                    ...
                    fetch(process.env.REACT_APP_API_URL + apiEnd, requestOptions)
                        ...
                        .then((resp) => {
                            ...
                            if (resp.success === true) {
                                if (this.state.id !== "") {
                                    var successMsg = 'Make has been updated successfully';
                                }
                                else {
                                    successMsg = 'Make has been created successfully';
                                }
                ...
                <form onSubmit={this.handleSubmit}>
                    <div className="col ml-12">
                        <div className="form-group row">
                            <label className="col-lg-2 col-form-label" style={{ "textAlign": "center" }}>Make Name<span className="required text-danger"> *</span></label>
